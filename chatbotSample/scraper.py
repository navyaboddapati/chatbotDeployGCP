...
def create_tables(conn):
    ...
    # Create ResearchCollection table
    c.execute('''CREATE TABLE IF NOT EXISTS ResearchCollection
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  text TEXT,
                  link TEXT)''')

    # Create LocationCollection table
    c.execute('''CREATE TABLE IF NOT EXISTS LocationCollection
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  library TEXT,
                  link TEXT)''')

    # Create BookCollection table
    c.execute('''CREATE TABLE IF NOT EXISTS BookCollection
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  title TEXT,
                  publisher TEXT,
                  identifier1 TEXT,
    ...
    # Create NewsEventCollection table
    c.execute('''CREATE TABLE IF NOT EXISTS NewsEventCollection
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  date TEXT,
                  title TEXT,
                  link TEXT)''')

    # Create AskCollection table
    c.execute('''CREATE TABLE IF NOT EXISTS AskCollection
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  question TEXT,
                  link TEXT,
                  answer TEXT)''')
...
def scrape_data():
    ...
    if response.status_code == 200:
        ...
        if research_response.status_code == 200:
            ...
            for link in research_soup_links[:6]:
                ...
                c.execute("INSERT INTO ResearchCollection (text, link) VALUES (?, ?)",
                          (link.text, research))
        else:
    ...
    if location_response.status_code == 200:
        ...
        for link in location_soup_links[:3]:
            ...
            c.execute("INSERT INTO LocationCollection (library, link) VALUES (?, ?)",
                      (link.text, location_hours))
    else:
    ...
    for data in book_data:
        ...
        c.execute("INSERT INTO BookCollection (title, publisher, identifier1, identifier2, link) VALUES (?, ?, ?, ?, ?)",
                (data['Title'], data['Publisher'], identifier1, identifier2, data['Link']))
    ...
    if news_response.status_code == 200:
        ...
        for event in news_events:
            ...
            c.execute("INSERT INTO NewsEventCollection (date, title, link) VALUES (?, ?, ?)",
                      (news_time, news_title, news_link))
    else:
    ...
    if ask_response.status_code == 200:
        ...
        for iterate in ask_popular:
            ...
            if answer_response.status_code == 200:
                ...
                c.execute("INSERT INTO AskCollection (question, link, answer) VALUES (?, ?, ?)",
                          (ask_title, ask_link, answer_html))
            else:
